...
public class CommentService {
    ...
    public List<CommentForm> comments(Long articleId) {
        ...
        for(int i = 0; i < comments.size(); i++) {
            ...
            CommentForm dto = CommentForm.createCommentDto(c);
            dtos.add(dto);
        }
        // 3. 결과 반환
        return dtos;*/
        ...
        return commentRepository.findByArticleId(articleId) // 댓글 엔티티 목록 조회
                ...
                .map(comment -> CommentForm.createCommentDto(comment))  // 엔티티를 DTO로 매핑
                .collect(Collectors.toList()); // 스트림을 리스트로 변환
    }

    // 2. 댓글 생성
    @Transactional //DB 내용을 바꾸기 때문에 실패할 경우 대비
    ...
    public CommentForm update(Long id, CommentForm dto) {
        ...
        // 2. 댓글 수정
        target.patch(dto);
//        Comment target = Comment.createComment(dto);
        // 3. DB로 갱신
